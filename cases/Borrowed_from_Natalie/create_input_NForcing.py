...
if __name__ == '__main__':
    ...
    argv = sys.argv[2]  # date yyyy-mm-dd
    end_date = datetime.datetime.strptime(argv,'%Y%m%d')
    ...
    path = '/nobackup/users/theeuwes/LES_forcings/new_forcings/'
    path_e5 = '/nobackup/users/theeuwes/LES_forcings/'
    path_out = '/nobackup/users/theeuwes/DALES_runs/'
    path_out_r = '/nfs/home/users/theeuwes/work/DALES_runs/'
...
#        nc_files = get_file_list(path, date+offset, date+t_exp+eps)
        nc_files = '{0:}/LES_forcing_{1:04d}{2:02d}{3:02d}.nc'.\
        ...
        workdir_r = '{0}/{1:04d}{2:02d}{3:02d}'.format(path_out_r, date.year, date.month, date.day)
        ...
        create_runscript ('DALES{0:03d}_{1}'.format(expnr, n), ntasks, nnodes, walltime=35, work_dir=workdir_r, expnr=expnr)
        ...
        if start_is_warm:
            ...
            f_in  = '{0}/baseprof.inp.{1:03d}'.format(prev_workdir_r, expnr)
                ...
                for j in range(nl['run']['nprocy']):
                    for ftype in ['d','s','l']:
                        ...
                        f_in  = '{0}/init{1}{2:03d}h{3:02d}mx{4:03d}y{5:03d}.{6:03d}'\
                                    .format(prev_workdir_r, ftype, hh, mm, i, j, expnr)
