...
uam  - u (east-west) componant of the wind [m s-1]  
vam  - v (north-south) componant of the wind [m s-1] 
w    – w (vertical velocity) component of the wind [m s-1] 
...
ts_rad  - surface temperature

fp 3d variables 
phi - geopotential height  (I don't have it) !!!


"""
# Ignore this is just to pull data off ecmwf tapes
# def get_from_ecfs_daily(date,ecpath):
#     """
...
#     f = '{0:}/{1:04d}/{2:02d}/{3:02d}/{4:02d}/'


# def get_from_ecfs_monthly(date,ecpath):
#     """
...
class harmonie_to_LESforce:
    ...
    def __init__(self, path, date, lat, lon, buffer=0, add_soil=False, dt=60, quiet=False):
 
        sigma = (pd.read_csv('H43lev65.txt',header=None,index_col=[0],delim_whitespace=True))[2].values[:-1]
        
        self.nlev  = len(sigma)  # Number of full vertical levels
        self.nlevh = self.nlev + 1                # Number of half vertical levels
        self.nt    = int(24)              # Number of output time steps
        ...
        self.w    = np.ma.zeros(dim3d)  # w–vertical velocity (m s-1)
        ...
        self.p_s  = np.ma.zeros(dim2d)  # Surface pressure (Pa)
        self.huss  = np.ma.zeros(dim2d)  # Surface specific humidity (kg kg-1)
        ...
        if True: 
            ...
            for varstr in tend_names:
                ...
                f = '{0}{1}_Slev_his_EUREC4Acircle_BES_harm40h111_fERA5_noASSIM_LL48FCINT24_{2:04d}{3:02d}{4:02d}{5:02d}_1hr.nc'.\
                format(path,varstr, date.year, date.month, date.day, date.hour)                
                if os.path.exists(f):
                    files.append(f)
                else:
            ...
            # other 3D fields 
            vars_3d_state = ['hus', 'cli', 'clw', 'ta','uam','vam']
            files = []
            ...
            if (buffer>0) : 
                self.u = np.nanmean(nc.uam.values[:,:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(2,3))
                self.v = np.nanmean(nc.vam.values[:,:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(2,3))
                ...
                self.u = nc.uam.values[:,:,j,i]
                self.v = nc.vam.values[:,:,j,i]
            ...
            # self.hours_since = np.array([(datetime.utcfromtimestamp(t.astype(int)*1e-9)-datetime(2010,1,1)).total_seconds()/3600. for t in self.time])
            ...
            for varstr in vars_fp3d:
                f = '{0}{1}_Slev_fp_EUREC4Acircle_BES_harm40h111_fERA5_noASSIM_LL48FCINT24_{2:04d}{3:02d}{4:02d}{5:02d}_1hr.nc'.\
                format(path,varstr, date.year, date.month, date.day, date.hour)  
            ...
            if (buffer>0) : 
                self.p = np.nanmean(nc.p.values[:,:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(2,3))
                self.w = np.nanmean(nc.w.values[:,:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(2,3))
                # self.z = np.nanmean(nc.phi.values[:,:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(2,3))
            else : 
                self.p = nc.p.values[:,:,j,i]
                self.w = nc.w.values[:,:,j,i]
                # self.z = nc.phi.values[:,:,j,i]

            # Generate list of NetCDF files to read
            # 2d fields (non-sfx)
            vars_nearsurf = ['ps','ts_rad','huss']

            files = []
            for varstr in vars_nearsurf:

                f = '{0}{1}_his_BES_BES_harm40h111_fERA5_noASSIM_LL48FCINT24_{2:04d}{3:02d}{4:02d}{5:02d}_1hr.nc'.\
                format(path,varstr, date.year, date.month, date.day, date.hour)  
            ...
            if (buffer>0) : 
                ps     = np.nanmean(nc.ps.values[:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(1,2))
                ts_rad = np.nanmean(nc.ts_rad.values[:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(1,2))
                huss   = np.nanmean(nc.huss.values[:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(1,2))
  
            else : 
                ps     = nc.ps.values[:,j,i]
                ts_rad = nc.ts_rad.values[:,j,i]
                huss = nc.huss.values[:,j,i]
            # self.p = np.array([p * sigma for p in ps ])
            self.p_s = ps
            self.Tsk = ts_rad
            self.huss = huss
            ...
            # Generate list of NetCDF files to read
            # 2d fields (non-sfx)
            vars_nearsurf = ['hfss','hfls']

            files = []
            for varstr in vars_nearsurf:
                # !!!for some reason these variables are saved from 01am so have to open the file of the day before (day-1)
                f = '{0}{1}_his_BES_BES_harm40h111_fERA5_noASSIM_LL48FCINT24_{2:04d}{3:02d}{4:02d}{5:02d}_1hr.nc'.\
                format(path,varstr, date.year, date.month, date.day-1, date.hour)  
            ...
            if (buffer>0) : 
                self.H  = np.nanmean(nc.hfss.values[:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(1,2))/dt
                self.LE = np.nanmean(nc.hfls.values[:,j-buffer:j+buffer,i-buffer:i+buffer],axis=(1,2))/dt
                

            else : 
                self.H  = nc.hfss.values[:,j,i]/dt
                self.LE = nc.hfls.values[:,j,i]/dt
        ...
        self.dtw_off  = self.calc_tendency(self.w,  dt)
    ...
    def to_netcdf(self, file_name):
        ...
        f.setncattr('model_id', "harmonie-40h1.1.1")
        f.setncattr('domain', "BES")
        f.setncattr('driving_model_id', "ERA5")
        f.setncattr('experiment_id', "BES_harm4h111_fERA5_noASSIM_LL48FCINT24")
        f.setncattr('title', " - initial & boundary conditions for LES")
        f.setncattr('project_id', "")
        f.setncattr('institution', "Royal Netherlands Meteorological Institute, De Bilt, The Netherlands")
        f.setncattr('data_contact', "Alessandro Savazzi, TU Delft (A.C.M.Savazzi@tudelft.nl)")
        ...
        add_variable(f, 'w',    dtype, dim3d, False, {'units': 'm s-1',   'long_name': 'Vertical velocity'}, self.w)
        ...
        add_variable(f, 'q_s',     dtype, dim2d, False, {'units': 'kg kg-1', 'long_name': 'Surface specific humidity'}, self.huss)
        # add_variable(f, 'p_s',     dtype, dim2d, False, {'units': 'Pa',      'long_name': 'Surface pressure'}, self.ph[:,:,-1])
        add_variable(f, 'p_s',     dtype, dim2d, False, {'units': 'Pa',      'long_name': 'Surface pressure'}, self.p_s)
...
if __name__ == '__main__':
    ...
    read_dir  = os.path.abspath('{}/../extracted/')+'/'
    write_dir = os.path.abspath('{}/../LES_forcing/')+'/'
    
    st_date = datetime(2020,2,5)
    en_date = datetime(2020,2,6)
    dt_exp  = timedelta(hours=24)
    
    date    = st_date
    while date <=en_date:
    
        data = harmonie_to_LESforce( read_dir, date, lat_select, lon_select, buffer = buffer, add_soil=False, dt=step*dt, quiet=False)
    
        data.to_netcdf(write_dir+'LES_forcing_'+date.strftime("%Y%m%d%H")+'.nc')
        print(date.strftime("%Y–%m–%d:%H")+' saved.')
        
        date += dt_exp
    # os.system( "say beep, I have done" )
